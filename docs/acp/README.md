...
- Approval-blocked executions are exposed as strict `status: "awaiting"` runs with an `await.schema` and `await.prompt` payload that Nexus can render on Slack, Telegram, WhatsApp, email, and webchat. `POST /v1/acp/runs/{run_id}/resume` accepts a Nexus resume payload with `choice`, `decision`, `action`, or `value` set to approve/reject and resolves the pending Parmesan approval before returning the updated run.
...
- Session creation accepts `allow_greeting: true` to queue an agent greeting immediately after the session is created. Greeting works in `auto`, `manual`, `supervised`, and `unattended` sessions and is deduped per session.
- Session creation accepts `allow_first_message_response: true` to let a `manual` session create one automated execution for its first customer message. Later customer messages in the same manual session are still persisted without automation until an operator explicitly processes them. Supervised sessions do not need this flag because every customer message already creates a draft execution.
...
- Location-capable gateway integrations can send Nexus-compatible location parts using `content_type: "application/vnd.nexus.location+json"` and a JSON `content` body with `latitude`, `longitude`, and optional `name`/`address`; `content` may be either a JSON object or an escaped JSON string. Parmesan also accepts equivalent `structured_data` parts with `data.kind: "location"`. Location-only messages are accepted: Parmesan renders a coordinate/maps-link text fallback for the customer message and injects the same active-turn location as trusted location context during response composition.
...
- If the session mode is `supervised`, ACP message ingress creates the normal automated execution, but `deliver_response` marks the generated response `review_required` with reason `supervised_review` instead of delivering it directly. Held draft message events are hidden from ACP list/stream responses until an operator approves, edit-forwards, or replaces the draft through the response review endpoints.
- Live provider validation for supervised mode can be run with `PARMESAN_LIVE_OPENROUTER=1 OPENROUTER_API_KEY=... go test ./internal/engine/runner -run TestLiveOpenRouterSupervisedModeGeneratesHeldDraft -count=1`; it verifies OpenRouter generation, `review_required` storage, and ACP hiding before approval.
- If the session mode is `unattended`, automated execution proceeds, but approval-required tools are auto-approved only when their policy has opted into unattended approval; other required tools follow the bundle's unattended ineligible-tool behavior. The same opt-in does not bypass approval in `auto`, `manual`, or `supervised` mode.
...
- Non-text media ACP content parts with URLs are treated as media assets; image/audio parts now produce derived signals like OCR text, summaries, labels, transcripts, and language hints. Structured email context, Nexus-compatible location parts, and `artifact_ref` parts are preserved as message context and are not sent through media enrichment.
